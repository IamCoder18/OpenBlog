...
describe("POST /api/posts - Integration Tests", () => {
  ...
  describe("13. SQL injection prevention", () => {
    it("should reject title with SQL keywords", async () => {
      const { user } = await createTestUser({ email: "sql1@test.com" });

      const response = await makeRequest(
        {
          title: "Test SELECT FROM posts",
          slug: "sql-title-test",
          bodyMarkdown: "# Content",
        },
        user.id
    ...
    it("should reject slug with SQL keywords", async () => {
      const { user } = await createTestUser({ email: "sql2@test.com" });

      const response = await makeRequest(
        {
          title: "Test",
          slug: "test-slug-union-select",
          bodyMarkdown: "# Content",
        },
        user.id
      );
    ...
    it("should reject bodyMarkdown with SQL keywords", async () => {
      const { user } = await createTestUser({ email: "sql3@test.com" });
    ...
    it("should accept valid content with similar words", async () => {
      const { user } = await createTestUser({ email: "sql4@test.com" });

      const response = await makeRequest(
        {
          title: "My Selected Article",
          slug: "selected-article",
          bodyMarkdown: "# This article selects the best topics",
        },
        user.id
      );
